...
import 'orderbook/order_event_handler.dart';
import 'util.dart';
import 'vitex/proto/dex_order.pbserver.dart' hide TokenInfo;
...
@freezed
class BlockEvent with _$BlockEvent {
  const BlockEvent._();
  const factory BlockEvent({
    required List<OrderEvent> orderEvents,
    required int height,
  ...
  factory BlockEvent.create(
    ...
    return BlockEvent(
      ...
      height: height,
      hash: hash,
      tradePair: orderEvents.first.orderLog!.tradePair,
    );
  }

  factory BlockEvent.fromAccBlockVmLogs(
    AccBlockVmLogs accBlockVmLogs,
    Tokens tokens,
  ) {
    final vmLogs = accBlockVmLogs;
    vmLogs.parseTransactions();

    final events = <OrderEvent>[];
    for (final vmLog in vmLogs.vmLogs) {
      final hash = vmLog.accountBlockHash;
      final orderEvent = OrderEvent.parse(
        vmLog: vmLog.vmLog,
        vmLogs: vmLogs,
        tokens: tokens,
        hash: hash.hex,
    ...
    final result = BlockEvent.create(
      events,
      vmLogs.height,
      vmLogs.hash,
    );
  ...
  void travel(OrderEventHandler handler, {required bool inReverse}) {
    if (inReverse) {
      for (final event in orderEvents.reversed) {
        handler.revertOrderEvent(event);
      }
      return;
    }
    for (final event in orderEvents) {
      handler.onwardOrderEvent(event);
    }
  }
}
